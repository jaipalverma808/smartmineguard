...
class MaterialMonitoringService:
    ...
    def get_daily_dispatch_summary(mine_id=None, mineral=None):
        ...
        """
        # Dynamic calculation from database records with fallback to baselines
        if mine_id:
        ...
        # Actual dispatched quantity calculated from verified weighments today (scoped to today's date)
        ...
        # Fallback to latest operational day if no weighments logged for today
        ...
        # Pending weighment count (trips dispatched without recorded weighment)
        sql_pending = f"""
            SELECT COUNT(*) as c
            FROM trips tr
            LEFT JOIN weighments w ON w.trip_id = tr.id
        ...
        # Count of distinct trucks with quantity anomalies
        sql_anom = f"""
    ...
    def get_stock_reconciliation(mine_id=None):
        """
        Calculates the explicit stock reconciliation balance:
        Opening Stock + Today's Production - Today's Dispatch = Current / Estimated Stock
        """
    ...
    def get_mine_wise_material_summary():
        """
        Returns a comparative material and dispatch summary for all authorized mines (for ADMIN).
        """
    ...
    def get_truck_wise_material_ledger(mine_id=None, mineral=None, status_filter=None, search_query=None):
        """
        Returns the truck-wise material movement ledger with per-trip and cumulative stats.
        Columns: Vehicle No, Mine, Mineral, Permit No, Permitted Qty, Actual Qty, Difference,
                 Trips Today, Last Dispatch Time, Status.
        """
        # First gather cumulative stats per truck
        sql_cumulative = """
            SELECT 
                tr.truck_id,
        ...
        for r in trips:
            ...
            if last_time:
                last_time_str = str(last_time)[:16]
            else:
                last_time_str = "—"
    ...
    def get_quantity_anomalies(mine_id=None):
        """
        Returns deterministic material quantity anomalies where actual_weight > permitted_weight.
        Includes explainable excess tonnage, mine, destination, timestamp, and status.
        Used by the Enforcement Officer and Administrator dashboards.
        """
        ...
        # Count repeat anomalies per truck
        counts = {}
        for r in rows:
        ...
        for r in rows:
            ...
            if pct >= 20.0:
                status = "FLAGGED FOR INTERCEPTION"
            else:
    ...
    def get_top_material_rankings(mine_id=None):
        """
        Calculates rankings for top material movement:
        1. Top trucks by transported quantity today
        2. Top mines by dispatch quantity today
        3. Top trucks by number of trips today
        4. Top trucks by excess quantity
        """
        mine_filter = "WHERE tr.mine_id = ?" if mine_id else ""
        params = (mine_id,) if mine_id else ()

        # 1. Top trucks by transported quantity
        sql_trucks_qty = f"""
        ...
        # 2. Top mines by dispatch quantity
        sql_mines_qty = """
        ...
        # 3. Top trucks by number of trips
        sql_trucks_trips = f"""
    ...
    def get_mineral_wise_summary(mine_id=None):
        """
        Returns mineral-wise monitoring breakdown:
        Mineral, Produced MT, Dispatched MT, Remaining MT, Number of Trucks, Total Trips.
        """
        minerals_meta = {
    ...
    def get_dispatch_vs_production_timeseries(mine_id=None):
        """
        Returns a daily timeseries for Production vs Dispatch chart on the analytics view.
        """
    ...
    def get_dispatch_control_planning(mine_id=None):
        """
        Requirements 10 & 11: Daily Dispatch Planning and Control.
        Tracks planned dispatch, actual dispatch, remaining dispatch,
        number of trucks planned, number of trucks dispatched,
        expected rounds, completed rounds, and remaining rounds.
        """
        summary = MaterialMonitoringService.get_daily_dispatch_summary(mine_id=mine_id)
        ...
        # Trucks and rounds calculation
        mine_filter = "WHERE assigned_mine_id = ?" if mine_id else ""
        params = (mine_id,) if mine_id else ()

        truck_counts = db.query(f"""
            SELECT 
                COUNT(*) as planned_trucks,
                COALESCE(SUM(completed_rounds_today), 0) as completed_rounds
            FROM trucks
            {mine_filter}
        """, params, one=True)
    ...
    def get_truck_cumulative_material_profile(truck_id):
        """
        Requirement 12: Truck Material Cumulative Total Profile.
        Calculates:
        - Today's total material, trips, permitted, excess
        - This week's total material, trips, permitted, excess
        - This month's total material, trips, permitted, excess
        - Current permit quantity and current trip quantity
        - Cumulative lifetime dispatched quantity and total excess quantity detected
        - Allowed, completed, and remaining rounds
        """
        ...
        rows = db.query("""
            SELECT 
                tr.id, tr.start_time, p.permitted_weight_mt, w.net_weight_mt
            FROM trips tr
        ...
        return {
            ...
            "today": {
                "trips": today_trips,
                "material_mt": round(today_material, 1),
                "permitted_mt": round(today_permitted, 1),
                "excess_mt": round(today_excess, 1)
            },
            "week": {
                "trips": week_trips,
                "material_mt": round(week_material, 1),
                "excess_mt": round(week_excess, 1)
            },
            "month": {
                "trips": month_trips,
                "material_mt": round(month_material, 1),
                "excess_mt": round(month_excess, 1)
            },
    ...
    def get_drone_dem_volumetric_audit(mine_id=1):
        """
        Scenario 5: Drone LiDAR / DEM Volumetric Over-Extraction Audit.
        Reconciles physical pit excavation models (DEM surface differentials) against
        cumulative e-Ravanna dispatches to detect off-the-record over-extraction beyond lease limits.
        """
        ...
        # Query all sub-mine concession plots for this mine
        ...
        # In-situ rock density: Quartzite 1.62 MT/m3
        density = 1.62
        # Volumetric DEM differential calculation (Cumulative excavated pit void)
        # In actual quarrying, physical excavation exceeds official e-Ravannas by 12-18% when fraud occurs
        ...
        for b in qbs[:8]:  # Detailed top 8 plots
            ...
            plot_audits.append({
                ...
                "businessman": b["leaseholder_name"],
                "allocated_quota_mt": alloc,
                "dispatched_erawana_mt": disp,
                "dem_pit_volume_m3": plot_dem_vol_m3,
                "physical_extracted_mt": plot_physical_mt,
                "unaccounted_extraction_mt": plot_unaccounted_mt,
                "quota_utilization_pct": disp_pct,
                "kill_switch_status": "LOCKED" if is_breached else "ACTIVE",
    ...
    def get_crusher_inward_kacha_maal_audit(mine_id=1):
        """
        Scenario 4: Downstream Crusher Gate & Kacha Maal Reconciliation Audit.
        Reconciles downstream stone crusher feedstock intake against inbound e-Ravanna passes
        to identify off-record cash bypass and token transactions.
        """
        crusher_plants = [
            {
                "crusher_id": "CR-ALW-01",
                "name": "Aravalli Blue Metal Crusher Zone #1",
                "owner": "Ramesh Sharma Aggregates",
                "energy_consumed_kwh": 34800.0,
                "estimated_crushed_mt": 12428.5,  # ~2.8 kWh per MT aggregate
                "inward_erawana_mt": 10250.0,
                "unaccounted_kacha_maal_mt": 2178.5,
                "kacha_maal_rate_pct": 17.5,
                "evaded_gst_royalty_inr": 816937.5,
                "status": "VIGILANCE_INTERCEPT"
            },
            {
                "crusher_id": "CR-ALW-02",
                "name": "Mewat Highway Grit & Ballast Mill",
                "owner": "Abdul Hameed Stone Works",
                "energy_consumed_kwh": 26400.0,
                "estimated_crushed_mt": 9428.0,
                "inward_erawana_mt": 8100.0,
                "unaccounted_kacha_maal_mt": 1328.0,
                "kacha_maal_rate_pct": 14.1,
                "evaded_gst_royalty_inr": 498000.0,
                "status": "VIGILANCE_INTERCEPT"
            },
            {
                "crusher_id": "CR-ALW-03",
                "name": "Siliserh Heavy Stone Crushing Ltd.",
                "owner": "Mahesh Singhal Granites",
                "energy_consumed_kwh": 18900.0,
                "estimated_crushed_mt": 6750.0,
                "inward_erawana_mt": 6550.0,
                "unaccounted_kacha_maal_mt": 200.0,
                "kacha_maal_rate_pct": 2.9,
                "evaded_gst_royalty_inr": 75000.0,
                "status": "COMPLIANT"
            },
            {
                "crusher_id": "CR-ALW-04",
                "name": "Rajputana Highway Ballast Plant",
                "owner": "Bhanwar Singh & Sons",
                "energy_consumed_kwh": 31200.0,
                "estimated_crushed_mt": 11142.8,
                "inward_erawana_mt": 9400.0,
                "unaccounted_kacha_maal_mt": 1742.8,
                "kacha_maal_rate_pct": 15.6,
                "evaded_gst_royalty_inr": 653550.0,
                "status": "VIGILANCE_INTERCEPT"
            }
        ]
        ...
        return {
            "mine_id": mine_id,
            "crusher_cluster": "Alwar District Central Crushing Belt",
            "total_crusher_plants_monitored": len(crusher_plants),
            "total_crushed_feedstock_mt": total_crushed_mt,
            "total_erawana_intake_mt": total_erawana_mt,
            "total_unaccounted_kacha_maal_mt": total_kacha_maal_mt,
            "overall_kacha_maal_pct": overall_leakage_pct,
            "total_evaded_royalty_gst_inr": total_evaded_inr,
    ...
    def get_active_pit_dwell_watchdog(mine_id=1, max_dwell_minutes=90):
        """
        Scenario 6: Active Pit Dwell & Ghost Truck Interception Watchdog.
        Monitors all transport vehicles currently inside the mine perimeter. Flags excessive dwell times
        exceeding 90 minutes to prevent credential swapping, off-book shuttle runs, or ghost passes.
        """
        now = datetime.now()
        trucks = db.query("""
        ...
        # If no trucks currently flagged inside in local DB state, seed realistic live vehicles
        if not trucks:
            sample_vehicles = [
            ...
            for sv in sample_vehicles:
                ...
                if dwell > 180:
                    ...
                    status = "GHOST_HAUL_RISK"
                    ghost_count += 1
                elif dwell > 60:
                    status = "LOADING_CAUTION"
                    caution_count += 1
                else:
                    status = "NORMAL"
            ...
            for t in trucks:
                ...
                active_vehicles.append({
                    ...
                    "permit_number": t.get("permit_number") or "N/A",
                    "quarry_sector": "Central Concession Basin"
                })

        return {
            "mine_id": mine_id,
            "max_sanctioned_dwell_minutes": max_dwell_minutes,
            "total_trucks_inside": len(active_vehicles),
            "ghost_trucks_flagged": ghost_count,
